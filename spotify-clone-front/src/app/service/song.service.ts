...
export class SongService {
  ...
  add(song: SaveSong) : void {
    ...
    this.http.post<SaveSong>(`${environment.API_URL}/api/songs`, formData)
      .subscribe({
        next: saveSong => this.add$.set(State.Builder<SaveSong, HttpErrorResponse>().forSuccess(saveSong).build()),
    ...
    this.http.post<ReadSong>(`${environment.API_URL}/api/songs/like`,{favorite, publicId})
      .subscribe({
        ...
        error: err => {
          ...
          this.toastService.show("Error when adding song to favorites", "DANGER");
        }
      })
  }
  ...
  fetchFavorites() : void {
    this.http.get<Array<ReadSong>>(`${environment.API_URL}/api/songs/like`)
      .subscribe({
        next: favoriteSongs => this.fetchFavoriteSongs$.set(State.Builder<Array<ReadSong>, HttpErrorResponse>().forSuccess(favoriteSongs).build()),
        error: err => this.fetchFavoriteSongs$.set(State.Builder<Array<ReadSong>, HttpErrorResponse>().forError(err).build())
      })
  }
