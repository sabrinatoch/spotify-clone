...
@Component({
  selector: 'app-search',
  standalone: true,
  imports: [FormsModule, FontAwesomeModule, SmallSongCardComponent, FavoriteSongBtnComponent],
...
export class SearchComponent {
  ...
  onSearch(newSearchTerm: string) {
    ...
    of(newSearchTerm).pipe(
      tap(song => this.resetResultIfEmptyTerm(newSearchTerm)),
  ...
  onPlay(firstSong: ReadSong): void {
    this.songContentService.createNewQueue(firstSong, this.songsResults);
  }

  private onNext(searchState: State<Array<ReadSong>, HttpErrorResponse>) {
    this.isSearching = false;
    if (searchState.status === "OK")
      this.songsResults = searchState.value!;
