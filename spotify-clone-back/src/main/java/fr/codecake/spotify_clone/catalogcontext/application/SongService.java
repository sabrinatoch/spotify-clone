...
import fr.codecake.spotify_clone.infrastructure.service.dto.StateBuilder;
...
public class SongService {
    ...
    public List<ReadSongInfoDTO> search(String searchTerm) {
        List<ReadSongInfoDTO> searchSongs = songRepository.findByTitleOrAuthorContaining(searchTerm)
                .stream()
                .map(songMapper::songToReadSongInfoDTO)
                .collect(Collectors.toList());
        if (userService.isAuthenticated())
            return fetchFavoriteStatusForSongs(searchSongs);
        return searchSongs;
    }
